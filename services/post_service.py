...
#     # 첫 요청 시 (cursor가 없는 경우)
#     if not cursor_created_at or not cursor_post_id:
#         query = query.order_by(post_model.Post.created_at.desc(), post_model.Post.post_id.desc()).limit(limit + 1)

#     # 커서 필터링이 있는 경우
#     else:
#         query = query.filter(
#             or_(
#                 post_model.Post.created_at < cursor_created_at,
#                 and_(
#                     post_model.Post.created_at == cursor_created_at,
#                     post_model.Post.post_id < cursor_post_id
#                 )
#             )
#         ).order_by(post_model.Post.created_at.desc(), post_model.Post.post_id.desc()).limit(limit + 1)

#     # 쿼리 실행
#     posts = query.all()

#     # `has_more` 플래그를 설정하기 위해 `limit + 1` 만큼 조회
#     has_more = len(posts) > limit
#     if has_more:
#         posts = posts[:limit]  # 초과된 한 개의 데이터는 잘라낸다.
...
#     if posts:
#         # 마지막 게시물의 created_at과 post_id를 사용하여 next_cursor를 결정
#         last_post_created_at = posts[-1][0].created_at
#         last_post_id = posts[-1][0].post_id

#         # 동일한 created_at이 여러 개 있을 경우, 가장 작은 post_id를 찾아서 next_cursor로 설정
#         next_cursor_created_at = last_post_created_at
#         next_cursor_post_id = min(
#             post[0].post_id for post in posts if post[0].created_at == last_post_created_at
#         )
