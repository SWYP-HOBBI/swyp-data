...
def search_posts(db: Session, req: SearchRequest) -> SearchResults:
    ...
    for post, comment_count in rows:
        ...
        if req.mbti:
            # MBTI 필터가 적용되었고 이 post가 그 필터 조건에 포함됐을 때
            # (해당 post.user.mbti가 패턴에 맞는 경우)
            matched_fields.append("MBTI")
        if hobby_tag_ids:
            # 이 post가 위에서 변환한 hobby_tag_ids 조건을 만족하면
            matched_fields.append("태그")
